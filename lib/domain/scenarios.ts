/**
 * 24 synthetic health-insurance contact-centre calls used to seed a demo Knowledge Box and the
 * in-process mock ARAG server. Nothing here is real member data.
 */
...
/** Render a scenario as the plain-text transcript that is ingested / transcribed. */
export function transcriptOf(sc: Pick<Scenario, "turns">): string {
  return sc.turns.map((t) => `${t.speaker}: ${t.text}`).join("\n\n");
}

/** Deterministic duration estimate (the mock places one paragraph every 6 s). */
export function estimatedDurationSec(sc: Pick<Scenario, "turns">): number {
  return sc.turns.length * 6;
}
