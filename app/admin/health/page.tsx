...
import { AdminShell, JsonView, KeyValues, Panel, StateBlock, StatusPill, useAdminData } from "@/components/admin/AdminShell";
...
export default function AdminHealthPage() {
  ...
  return (
    <AdminShell
      ...
      actions={<Button variant="secondary" onClick={reload}>Re-test connection</Button>}
    >
      <StateBlock loading={loading} error={error}>
        {data && (
          <div className="space-y-4">
            <Panel
              title="Knowledge Box"
              right={<StatusPill ok={data.arag.ok} label={data.arag.ok ? `OK · ${data.arag.ms} ms` : "Unreachable"} />}
            >
              <KeyValues
                rows={[
              ...
              />
              {data.arag.error && (
                <p className="mt-3 rounded-md bg-danger-bg px-3 py-2 text-sm text-danger-fg">{data.arag.error}</p>
              )}
            </Panel>
